      ...
      <div className="flex-1 p-4 md:p-8 space-y-6 md:space-y-10 overflow-y-auto no-scrollbar">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            <h1 className="text-3xl md:text-5xl font-black text-[#042f21] tracking-tighter leading-tight font-outfit uppercase">Dashboard</h1>
            <p className="text-[#042f21]/40 text-sm md:text-base font-bold tracking-tight">Welcome back to Dephethogo Access.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex items-center gap-3 md:gap-4">
            <button className="flex items-center justify-center gap-2 px-6 py-4 bg-[#ccdbd4] border border-[#ccdbd4] rounded-2xl text-[13px] font-black text-[#042f21] hover:bg-[#b0c4ba] transition-all shadow-lg active:scale-95 uppercase tracking-widest min-h-[54px]">
              <QrCode size={18} strokeWidth={3} className="opacity-50" />
              Scan Code
            </button>
            <button className="flex items-center justify-center gap-2 px-8 py-4 bg-[#fa922c] text-white rounded-2xl text-[13px] font-black hover:bg-[#e07d20] transition-all shadow-xl shadow-[#fa922c]/30 active:scale-95 uppercase tracking-widest min-h-[54px]">
                ...
                <div key={log.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200/50 hover:bg-slate-100/80 hover:border-[#fa922c]/30 transition-all group flex items-center justify-between cursor-pointer">
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-3 items-center gap-2 md:gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-[#fa922c]/10 text-[#fa922c] flex items-center justify-center font-black text-sm md:hidden">
                        {log.initials}
                      </div>
                      <div className="flex flex-col min-w-0">
                        <p className="text-[15px] font-black text-[#042f21] leading-tight truncate">{log.visitor}</p>
                        <p className="text-[11px] font-bold text-[#042f21]/40 uppercase tracking-widest mt-1 md:hidden">On Site &bull; {log.site}</p>
                      </div>
                    </div>
                    <div className="hidden md:block text-[14px] font-bold text-[#042f21]/60">{log.site}</div>
